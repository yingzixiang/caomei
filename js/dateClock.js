...
var calUtil = {
  ...
  draw:function(OP){
    ...
    //绑定注释
    var textDate = "";
	  textDate="<p class='note-p-1'><i class='note-i-1'></i>阅读签到<i class='note-i-2'></i>缺勤 点击日期可阅读当天内容</p>";
	  textDate += "<p class='note-p-2'><i></i>周测日</p>";
	  $("#calendar").append(textDate);
    //绑定日历表头
    ...
    for (d = 0; d < 7; d++) {
     ...
     if(ifHasSigned){
     	if(iYear == myDate.getFullYear() && iMonth == myDate.getMonth()+1 && myMonth[w][d] == myDate.getDate() ){
     		htmls.push("<td class='thnum on now'>" + (!isNaN(myMonth[w][d]) ? myMonth[w][d] : " ") + "</td>");
     	}else{
     		htmls.push("<td class='thnum on'>" + (!isNaN(myMonth[w][d]) ? myMonth[w][d] : " ") + "</td>");
     	}
     }else if(ifUnSigned){
     		if(iYear == myDate.getFullYear() && iMonth == myDate.getMonth()+1 && myMonth[w][d] == myDate.getDate() ){
	     		htmls.push("<td class='thnum off now'>" + (!isNaN(myMonth[w][d]) ? myMonth[w][d] : " ") + "</td>");
	     	}else{
	     		htmls.push("<td class='thnum off'>" + (!isNaN(myMonth[w][d]) ? myMonth[w][d] : " ") + "</td>");
	     	}
     }else if(!isNaN(myMonth[w][d])) {
     		if(iYear == myDate.getFullYear() && iMonth == myDate.getMonth()+1 && myMonth[w][d] == myDate.getDate() ){
	     		htmls.push("<td class='thnum calendar_record now'>" + (!isNaN(myMonth[w][d]) ? myMonth[w][d] : " ") + "</td>");
	     	}else{
